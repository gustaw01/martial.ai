...
def test_run_algorithm():
    ...
    plagiarisms, plagiarisms_other_lang = run_algorithm(plagiarised_text, "pl") # Ten tekst jest w bazie danych, powinien być wykryty jako plagiat

    with open("test.json", "w") as f:
        json.dump(plagiarisms, f) 

    plagiarisms_t, plagiarisms_other_lang_t = run_algorithm(plagiarised_text_translated, "fr") # Ten tekst jest w bazie danych, ale w innym języku
    plagiarisms_ot, plagiarisms_other_lang_ot = run_algorithm(original_text, "pl") # tego tekstu nie ma w bazie danych
